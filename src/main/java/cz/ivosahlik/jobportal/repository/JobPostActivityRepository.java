...
import cz.ivosahlik.jobportal.entity.IRecruiterJobs;
...
public interface JobPostActivityRepository extends JpaRepository<JobPostActivity, Integer> {
    ...
    @Query(value = """
            SELECT COUNT(s.user_id) as totalCandidates,j.job_post_id,j.job_title,l.id as locationId,l.city,l.state,l.country,c.id as companyId,c.name FROM job_post_activity j
    ...
    @Query(value = """
            SELECT * FROM job_post_activity j INNER JOIN job_location l on j.job_location_id=l.id  WHERE j.job_title LIKE %:job%
